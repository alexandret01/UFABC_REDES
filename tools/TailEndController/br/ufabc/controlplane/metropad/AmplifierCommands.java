...
public class AmplifierCommands {
	 ...
	 *
	 * */
	public static Command getCommandSetAGCGain(SerialNumber serial, double gain){
		int dado = (int)(gain * 10.0D);
		byte[] code = new byte[3];
        code[0] = Command.AMP_SET_AGC_GAIN;
        code[1] = (byte)(dado % 256);
	    code[2] = (byte)(dado / 256);
        Command command = getCommand(serial, code);
        return command;
	}
	 ...
	 *
	 * */
	private static Command getCommand(SerialNumber serial, byte[] code ){
		return new Command(serial, code, Servidor.getInstance().getNE(serial) instanceof Ne16Bit);
	}
