...
//						new Object[] { ChannelTable.channel2Wavelength(channelStringC + channel) * 1000000000.0D }) );
	...
	public static Command configureLambdaWDM(SerialNumber serial, String channel) throws DataPlaneException {
		...
		if (channelValue < initialChannel || channelValue > lastChannel) {
			throw new DataPlaneException("Tentantiva de configurar o canal " + channel + " fora dos limites permitidos: C" + initialChannel + "-C"
					+ lastChannel);
		}
		...
		if ((type == 'C') || (type == 'H')) {
			...
			// System.out.println("Data: "+Arrays.toString(cmdData));
			int commandId = Command.TRP_ITU_CHANNEL_LADO1;
			cmd = new Command(serial, commandId, cmdData);
		}
		return cmd;		
	}
